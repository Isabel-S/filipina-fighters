...
def parse_date_views(date_views):
    ...
    if len(split_string) > 1:
        view_count_string = split_string[1].strip()
    else:
        view_count_string = "0"
    view_count_int = 0
    ...
    view_count_string = view_count_string.replace(" views", "").replace(" View", "")
...
def extract_tordesillas(filename, newfile):
    videos = []
    with open(f'scraped_data/{filename}.html',"r", encoding="utf-8") as file:
        f = file.read()
    page = bs(f, 'lxml')
...
extract_tordesillas('tordesillas_vid_scraped', 'tordesillas_vid_extracted')
